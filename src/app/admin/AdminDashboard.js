...
export default function AdminDashboard({ initialLeads = [], initialCertificates = [], courses = [] }) {
  const [leads, setLeads] = useState(initialLeads);
  const [activeTab, setActiveTab] = useState("leads"); // leads or certificates
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex border-b border-gray-200">
        ...
        >
          <i className="fa-solid fa-users text-base"></i> Enquiries CRM
        </button>
        <button
        ...
        >
          <i className="fa-solid fa-certificate text-base"></i> Certificate Registry
        </button>
      </div>
      ...
      {activeTab === "certificates" && (
        <CertificateDashboard initialCertificates={initialCertificates} courses={courses} />
      )}
    </div>
  );
}
